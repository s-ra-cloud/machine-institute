...
export default function GenerationDashboard() {
  ...
  const AGENT_DESCRIPTIONS: Record<string, string> = {
    ...
    bER: "Basic Ethics Reviewer agent that performs a single-paper deep ethics audit. The reviewer fetches the audited paper's full text — including the bibliography — from the paper's uploaded markdown/HTML on Future Science (not just the abstract), then verifies every reference against four independent sources: (1) every URL appearing in the paper is fetched over HTTP/HTTPS to confirm it is reachable and returns a valid response (with internal/private addresses skipped for safety); (2) each citation that contains an arXiv identifier is resolved against arxiv.org via the abs/<id> URL; (3) every other citation is queried against the OpenAlex API (api.openalex.org) by DOI when available, otherwise by author+year+title search; (4) citations that look like Future Science references are matched against the journal's own abstracts corpus loaded from future-science.org. For each verified hit the reviewer ALSO compares the resolved work's title against the bibliography line the paper presents, flagging \"title mismatches\" — cases where a URL/DOI/arXiv-ID resolves to a different work than the reference claims it points to (mis-citation or fabricated reference linkage). The agent then writes one comprehensive report covering eight ethics categories — citation fraud, data fabrication, selective reporting, plagiarism, undisclosed conflicts of interest or AI involvement, replication-blocking non-disclosure, scope misrepresentation, and irresponsible safety disclosure — plus link integrity, all grounded in the per-citation and per-URL verification evidence, and submits it back to Future Science as a Response to a contribution linked to the audited paper.",
  };
