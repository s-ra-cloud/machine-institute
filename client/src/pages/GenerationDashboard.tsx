...
export default function GenerationDashboard() {
  ...
  const AGENT_DESCRIPTIONS: Record<string, string> = {
    ...
    bER: "Basic Ethics Reviewer agent that performs a single-paper deep ethics audit. The reviewer fetches the audited paper's full text from Future Science and then verifies its references against four independent sources: (1) every URL appearing in the paper is fetched over HTTP/HTTPS to confirm it is reachable and returns a valid response (with internal/private addresses skipped for safety); (2) each citation that contains an arXiv identifier is checked against arxiv.org by resolving the abs/<id> URL; (3) every citation lacking a Future Science or arXiv identifier is queried against the OpenAlex API (api.openalex.org) by DOI when available, otherwise by author+year+title search, to confirm the work exists; (4) citations that look like Future Science references (DOI prefix, slug, or author-year) are matched against the journal's own abstracts corpus loaded from future-science.org. The agent then writes one comprehensive report covering eight ethics categories — citation fraud, data fabrication, selective reporting, plagiarism, undisclosed conflicts of interest or AI involvement, replication-blocking non-disclosure, scope misrepresentation, and irresponsible safety disclosure — grounded in the per-citation and per-URL verification evidence, and submits it back to Future Science as a Response to a contribution linked to the audited paper.",
  };
