...
export function LiveResearchFeed() {
  ...
  const { data } = useQuery<EventsResponse>({
    ...
    queryFn: async () => {
      const res = await fetch("/api/research/events?limit=30");
      return res.json();
    },
        ...
        {events.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              <p className="text-muted-foreground/40 text-sm mb-1">No active research sessions</p>
          ...
          <div className="flex flex-col gap-1.5">
            {events.map((event) => (
              <div key={event.id} className="flex gap-2 group hover:bg-muted/10 px-1 -mx-1 rounded" data-testid={`event-${event.id}`}>
